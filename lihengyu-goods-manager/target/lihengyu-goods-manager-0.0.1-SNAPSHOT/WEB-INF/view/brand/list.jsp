      ...
      <div class="modal-body">
        	<form id="updatespec">
        		 ...
        		 <div class="form-group">
    				...
    				<small id="specNamelHelp" class="form-text text-muted">请输入规格名称</small>
  					<label for="firstChar">首字母</label>
    				<input type="text" class="form-control" name="firstChar" id="upFirstChar" aria-describedby="specNamelHelp">
    				<small id="specNamelHelp" class="form-text text-muted">请输入首字母</small>
  				</div>
        	</form>
      </div>
	...
	<c:forEach items="${pageInfo.list}" var="brand">
		<tr>
			...
			<td>
				...
				<button type="button" class="btn btn-warning" onclick="toUpdate(${brand.id})">修改</button>
			</td>
...
<script type="text/javascript">
	...
	function toUpdate(id){
		...
		$.post("/brand/toUpdate",{id:id},function(data){
			//规格的id
			$("#upId").val(data.id)
	...
	*/
	function delBatch(){
		...
		if(confirm("您确认删除这些数据么？")){
			$.post("/brand/update",{ids:delIds},function(data){
				if(data="true"){
					alert("删除成功")
					refresh();
				}else{
